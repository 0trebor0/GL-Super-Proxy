                    ...
                    if (state.packet.Length >= 7)
                    {
                        ...
                        if (bytesAvailable >= bytesNeeded)
                        {
                            ...
                            if (state.GetType() == typeof(ClientState))
                            {
                                ...
                                if (ProxyUI.Instance != null)
                                    ProxyUI.Instance.AddPacket(ClientPacket.ID, ClientPacket.DecryptedPayload.Length, PacketDestination.FROM_CLIENT, DateTime.Now, ClientPacket.DecryptedPayload);
                            ...
                            else if (state.GetType() == typeof(ServerState))
                            {
                                ...
                                if (ProxyUI.Instance != null)
                                    ProxyUI.Instance.AddPacket(ServerPacket.ID, ServerPacket.DecryptedPayload.Length, PacketDestination.FROM_SERVER, DateTime.Now, ServerPacket.DecryptedPayload);
